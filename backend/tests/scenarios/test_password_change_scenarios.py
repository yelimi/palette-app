...
def _change(client, headers, body):
    return client.patch("/auth/password", json=body, headers=headers)
...
def test_TC66_패스워드_동일_입력(client):
    headers = _headers(client)
    response = _change(client, headers, {"current_password": BASE_PASSWORD, "new_password": BASE_PASSWORD})
...
def test_TC68_최대_길이의_새_패스워드(client):
    ...
    response = _change(client, headers, {"current_password": BASE_PASSWORD, "new_password": max_password})
    assert response.status_code == 200
...
def test_TC72_로그인_없이_변경_시도(client):
    response = client.patch("/auth/password", json={"current_password": BASE_PASSWORD, "new_password": "kimkim33@"})
...
def test_TC74_중간_빈_문자열의_새_패스워드(client):
    headers = _headers(client)
    response = _change(client, headers, {"current_password": BASE_PASSWORD, "new_password": "kim kim33@"})
    assert response.status_code == 200
